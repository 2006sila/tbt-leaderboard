...
门槛之外，机器人还会做一次**数值合理性检查**。挑的全是「物理不可能」级特征 ——
真实记录必然有帧率梯度、有方差、有事件；而伪造的记录文件（改 tbt 输出内容后
让 TBTS 重新导出）往往一路完美。

命中的成绩**照常上榜**，只是多一个 `suspicious` 标签（issue 上）和
「数值异常」角标（榜单上），展开详情能看到逐条命中项。**不拒收、不降权、不影响名次。**

榜单页的筛选栏有一个**「不看异常数据」**勾选框（默认不勾选）：勾上就把带角标的成绩
从表格里隐藏，计数栏写明隐藏了几条。只是不显示 —— 数据仍在榜、名次照算。勾选状态会
记在浏览器本地（localStorage），下次打开保持。

同时机器人会**提醒维护者**：回帖里 @ 一句，并把该 issue 指派给他 —— GitHub 会推送
站内与邮件通知，不需要额外配置。提醒对象默认为仓库 owner（可用 `sanity.notify.mention`
指定他人，`enabled: false` 可整体关掉）。

| 判据 | 依据 |
|---|---|
| 记录文件名含 `modified` / `fake` / `edit` / `copy` / `test` | 改过的文件常带这类后缀 |
| 记录文件名不符合 `sensors-*.jsonl(.gz)` | 不是 TBTS 的原始输出 |
| `fpsAvg == fps1LowAvg == fpsWorst1Pct` | 真实记录必然有梯度 |
| `fps1LowAvg > fpsAvg` 或 `fpsWorst1Pct > fps1LowAvg` | 逻辑上不可能 |
| `frametimeCv == 0` 且 `stutterPct == 0` 且 `eventCount == 0` | 真实运行不可能毫无抖动 |
| `latencyP99Ms < 1` | 低于物理下限 |
| 六项 `score.parts` 全部 ≥ 99.5 | 真实机器不可能项项满分 |

阈值与开关同样在 [`web/data/policy.json`](web/data/policy.json) 的 `sanity` 块里。

> 边界说明：这是**提高造假成本**，不是防伪。真正的核验必须拿到原始 `.jsonl.gz`
> 用服务端复算，那需要把评分逻辑抽成可复用的库 —— 目前没做。
> 所以榜单口径依然是"自报，未经复算核验"。
